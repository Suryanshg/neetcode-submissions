class Solution:
    def merge(self, intervals: List[List[int]]) -> List[List[int]]:
        ...
        for interval in intervals[1:]:
            ...
            # If current interval and iterator interval are merging
            # Merge interval into current interval
            if interval[0] <= current_interval[1]:
                current_interval[0] = min(current_interval[0], interval[0])
                current_interval[1] = max(current_interval[1], interval[1])
            
            # Else, there is no merge, so we append current interval to merged intervals
